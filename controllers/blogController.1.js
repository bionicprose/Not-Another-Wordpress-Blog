...
exports.blog__list = function(req, res) {
  ...
  Blog.find({'state': "publish", 'postDate':{$gt : lastWeek} })
    .sort({ createDate: "descending" })
    .exec(function(err, blogs) {
      res.render("blogs/index", { calBlogs: blogs, blogs: blogs, title: 'Bionic Prose', lastWeek: week });
    });
};

exports.blog__list__lastWeek = function(req, res) {

  var thisWeek = req.params.week;
  var prevWeek = 7 + Number(req.params.week);


  var tWeek = moment().subtract(thisWeek, 'days');
  var tWeekString = tWeek.toString();
  var pWeek = moment().subtract(prevWeek,'days');
  var pWeekString = pWeek.toString();
  var test = tWeek.month();
  console.log(test);
  var sMM;
  var eMM;
  tWeek.month() > 12 ? sMM = 1 : sMM = tWeek.month() + 1;

  var sDD = tWeek.date('DD');
  var sYYYY = tWeek.year();
  pWeek.month() > 12 ? eMM = 1 : eMM = pWeek.month() +1;
  var tryAgain = tWeek.toISOString().substr(0, 10);
  console.log(tryAgain);
  var tryAgainLastWeek = moment(tryAgain).subtract(7, 'days');
  var x = tryAgainLastWeek.toISOString().substr(0,10);
  console.log(tryAgainLastWeek);
  console.log('iso of talw: ' +x);
  var eDD = pWeek.date();
  var eYYYY = pWeek.year();
  var pastRange = sDD + '-' + sMM  + '-' + sYYYY+'--to--'+eDD+'-'+eMM+'-'+eYYYY;
  var newWeek = pastRange.slice(0, 10);
  
  newWeek = newWeek.replace(/-/g, ' ');
  console.log(newWeek);
  // var newWeekDated = moment(newWeek);
  console.log(pastRange);
  // console.log(newWeekDated);
  var lWeekISO = eYYYY+'-'+eMM+'-'+eDD;
  // var momented = moment(lWeekISO);
  // console.log(lWeekISO);
  // console.log(momented);


  Blog.find({'state':'publish', 'postDate' : {$gt: pWeek, $lt:tWeek}})
    .sort({createDate: 'descending'})
    .exec(function(err, blogs) {
      Blog.find({'state':'publish'})
        .sort({createDate:'descending'})
        .exec(function(err, calBlogs) {
          res.render('blogs/index', {calBlogs: blogs, blogs: blogs, lastWeek: pastRange, nextWeek: thisWeek });
        });
    });
}
